...
import os, time, uuid, re, json, io
import pandas as pd
...
def generate_booklets_drive(topics: list[str], booklet_title: str, make_citations: bool=True):
    ...
    for topic in topics:
        ...
        md = get_text_answer(ss["qe_google"],
    ...
    # overview & manifest
    overview_lines = [f"# {booklet_title}", "", f"생성시각: {time.strftime('%Y-%m-%d %H:%M:%S')}", ""]
