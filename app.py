# app.py — 관리자 초기 "완전 스캔(heavy) + 숫자 진행률 + 멈춤 감지 + ⬆️상단 고정 진행판 + 🔔소리 알림"
...
st.info("✅ 첫 방문(관리자)은 정확도를 위해 **완전 스캔**을 수행합니다. 진행률/지연 경고, 상단 고정 판과 알림음으로 상태를 확인할 수 있어요.")

# === Sticky 진행판 + 소리 알림 인젝션 ===================
def _inject_sticky_and_beeper():
    # CSS + WebAudio(브라우저에서 짧은 비프음)
    st.markdown("""
<style>
#sticky-progress{
...
@media (prefers-color-scheme: dark){
  #sticky-progress{ background: rgba(0,0,0,.45); }
}
.stk-row{ display:flex; flex-wrap:wrap; gap:8px; align-items:center; font-size:13px; }
.stk-pill{ padding:2px 8px; border-radius:999px; background:rgba(0,0,0,.05); border:1px solid rgba(0,0,0,.06);}
.stk-name{ max-width:52vw; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.stk-bar{ height:8px; width:260px; background:rgba(0,0,0,.08); border-radius:8px; overflow:hidden; }
.stk-fill{ height:100%; width:0%; background:linear-gradient(90deg,#4f46e5,#14b8a6); transition:width .18s ease; }
</style>
<script>
window.aiBeep = window.aiBeep || function(freq=880, dur=0.12) {
  try {
...
ss["_sticky_area"] = st.empty()  # 매 렌더마다 새 placeholder

def _sticky_update(pct:int, done:int, total:int, filename:str, note:str="", started_at:float|None=None):
    mmss = "00:00"
    if started_at:
        elapsed = int(time.time()-started_at)
        mm, ss_ = divmod(elapsed, 60); mmss = f"{mm:02d}:{ss_:02d}"
    html = f"""
...
"""
    ss["_sticky_area"].markdown(html, unsafe_allow_html=True)

def _beep(freq:int=880):
    # 일부 브라우저는 사용자 인터랙션 전 자동재생을 막을 수 있음(첫 클릭 이후부터 확실)
    st.markdown(f"<script>try{{aiBeep({freq});}}catch(e){{}};</script>", unsafe_allow_html=True)

_inject_sticky_and_beeper()
...
# ---- 관리자용(완전 스캔 + 진행률/워치독/취소 + Sticky + Beep) ----
def admin_full_scan_and_schedule(topics_text: str, title: str, cite: bool, rename_title: bool=False):
    ...
    # 상단 Sticky 진행판 + 컨트롤 박스
    _beep(660)  # 스캔 시작 알림
    _sticky_update(0, 0, max(len(metas),1), "준비 중…", "스캔 시작", time.time())
    ...
    for m in metas:
        ...
        try:
            ...
            _sticky_update(pct, done, total, name, "해시 실패", started)
            last_tick = time.time()
        ...
        _sticky_update(pct, done, total, name, f"신규/변경 {new_or_changed}", started)
...
with st.expander("📤 관리자: 자료 업로드 (원본→prepared 저장)", expanded=False):
    ...
    if st.button("업로드/가져오기 → prepared", type="primary", use_container_width=True):
        _beep(740)  # 사용자 클릭 시작 알림(브라우저 자동재생 해제 회피용)
        from googleapiclient.discovery import build
        from googleapiclient.http import MediaIoBaseUpload
