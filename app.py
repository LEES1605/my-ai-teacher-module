# app.py — 한 번의 인덱싱 + 두 LLM(Gemini/ChatGPT) 준비 + 각자 진행바
...
if st.button("🚀 한 번에 준비하기", use_container_width=True):
    # 0) 초기 상태
    _render_progress(g_bar, g_msg, 0, "대기 중…")
    _render_progress(o_bar, o_msg, 0, "대기 중…")

    # 1) 임베딩 공급자(공통) 선택 — 가성비 빠른 OpenAI 임베딩으로 권장
    ...
    # 5) LLM 두 개 준비 (각 바 별도로 마지막 단계 표시)
    # 5-1) Gemini
    try:
        g_llm = make_llm(
            provider="google",
...
if user_input:
    ...
    if qe is None:
        ...
        with st.spinner(f"{answer_with}가 답변을 생각하고 있어요..."):
            from src.rag_engine import get_text_answer
